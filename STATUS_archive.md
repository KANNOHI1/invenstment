# STATUS アーカイブ（2026-05〜2026-08）

通常は読まない。特定の過去判断の根拠を確認するときだけ参照する。現在地は `STATUS.md` を見ること。
...
- **2026-08-06、軌跡データにより判断を修正。`watchlist\trajectory_revision_2026-08-06.md`。** GitHub Actions経由で2ヶ月の日次終値を取得した結果、**9銘柄すべてが7/29（タカ派据え置きFOMCの日）に底を打ち、以降1週間の回復局面にあることが判明。NVDAは8/5に2ヶ月高値を更新。** これにより「倍率圧縮が終わる証拠を待つ、期限8/26」という前提は撤回（既に1週間前に終わっていた）。「借りる側 vs 稼ぐ側」の軸は高値回復度で裏づけられ有効（NVDA ±0% / MU -26% に対し IREN -36%）。**判断日を8/26から8/12（8/11引け後のCRWV決算後）へ前倒し。** NBIS売却は変更なし・根拠強化（+47.7%上昇した状態で高ハードルの決算に臨む形はAMDと同型）。
- **2026-08-06、レジーム判断を`watchlist\regime_decision_2026-08-06.md`に作成。結論: テーマは変えない。AIの中で「借りる側」から「稼ぐ側」へ軸を移し、倍率圧縮の終了を示す証拠が出るまで現金で待つ。期限は8/26のNVIDIA決算。** ①NBIS 12株は8/10-11に売却 ②別トレンドへのローテはしない（需要崩壊ではなく倍率圧縮であり、バリューへの乗り換えは3倍狙いの放棄に等しい）③IRENへの集約は信用スプレッドが落ち着くまで保留（市場が拒絶しているのは「資本市場に依存して成長するAI事業」であり、IREN -21%とMU +31%の差がその証拠）④現金は期限付きで保持。
- **2026-08-06、株価取得の構造的制約を確認しAGENTS.mdに明記。** Yahoo Finance等の金融APIは組織のegressポリシーで403、WebFetchも金融サイトは403、WebSearchのスニペットは日付混在で信頼できない（MODで17.8%の誤差が発生）。**巡回では株価を報告しない仕様に変更。株価が必要な判断はユーザーのアプリ画面を起点にする。**
...
## 2026-08-22 時点の「現在地」（2026-09-10 に STATUS.md から退避）[Claude]
...
**資金（2026-08-21 22:38アプリ確認）**: USD余力 **$4,335.01**＋円貨 **¥34,780**。予備費¥100万は口座外に確保済み。NISA枠2026年 残**¥19,965**＝使い切り扱い。
...
**執行計画は白紙撤回（2026-08-22 方針転換）。** レポートは市況・セクターローテ・主役・銘柄分析まで。**売買はユーザーが決定し、変更の連絡があったときだけ`holdings.json`を更新する。** 撤回した導出の記録は`zenpen_saihaichi_2026-08-21.md`／`tranche_zero_base_2026-08-21.md`（撤回ヘッダ付き）。
...
## 2026-09-18 に STATUS.md から退避した「判断済みの決定」11 件・「ハマりパターン」15 件 [Claude]

退避基準: CLAUDE.md に既に書いてある規律の重複、Codex 時代の運用に固有のもの、7〜8 月の一般論。環境起因・進行中タスクの前提は STATUS.md に残した。
